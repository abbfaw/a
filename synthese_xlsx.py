    ...
    def generate_xlsx_report(self, workbook, data, rapport):
        ...
        all_paie = payment.search(
            [('partenaire', '=', partner_id), ('status', '=', 'paye'),
             ('date_effective', '>=', start_date), ('date_effective', '<=', end_date)])
        ...
        all_date = all_orders.mapped('create_date') + all_paie.mapped("date_effective") + all_return.mapped("write_date")
        a = sorted(all_date)
        ...
        for x in a:
            ...
            var2 = payment.search(
                [("date_effective", '=', x), ('partenaire', '=', partner_id)])
            print('var2', var2)
            ...
            for order in var:
                if order.name not in tbl:
                    all_vals.update({
                        ...
                        'create_date': order.date_order.strftime('%Y-%m-%d')
                    })
            ...
            for enc in var2:
                if enc.name not in tbl:
                    ...
                    all_vals.update({
                        ...
                        'numero_bon': ' ',
                        'create_date': enc.date_effective.strftime('%d-%m-%Y'),
                    })
