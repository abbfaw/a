...
class RapportSecii(models.Model):
    ...
    def scrap_sale(self):
        print('Print Work -_-')
        data = {'id': self._context.get('active_id'), 'client': self.client.id, 'start_date': self.start_date.strftime('%d-%m-%Y'), 'end_date': self.end_date.strftime('%d-%m-%Y'), }
        return self.env.ref('account_secii.report_secii_synthese_partner').with_context(
            client_id=self.client.id).report_action(self, data=data)

    def collecte_donnees(self):
        # print('Print Work -_-')
        data = {'id': self.id, 'client': self.client.id, 'start_date': self.start_date, 'end_date': self.end_date, }
        print('Data =', data)
