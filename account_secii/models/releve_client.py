...
class SaleOrderInherit(models.Model):
    ...
    def recup_sale_rel_client(self):
        ...
        for so in sale_order.search([]):
            ...
            res = sale_order.search(
                [('partner_id', '=', so.partner_id.id), ('state', '=', 'sale'), ('is_prive', '=', True)])
            for x in res:
                vt = self.env['secii.vente'].sudo().search(
                    [('name', '=', x.name), ('partner_id', '=', x.partner_id.id)])
                for elt in vt:
                    vt.update({'amount_total': x.amount_total})
        self.suivi_up = True
...
class PurchaseOrderInherit(models.Model):
    ...
    def recup_purchase(self):
        ...
        for po in purchase_order.search([]):
            """Récupération de tous les achats avec l'état purchase et retour True avec en instance"""
            res = purchase_order.search(
                [('partner_id', '=', po.partner_id.id), ('state', '=', 'purchase'), ('retour', '=', True),
                 ('is_prive', '=', True)])
            for x in res:
                vt = self.env['secii.vente'].sudo().search(
                    [('name', '=', x.name), ('partner_id', '=', x.partner_id.id)])
                for elt in vt:
                    vt.update({'state': 'purchase', 'amount_total': -x.amount_total})
        self.suivi_up = True
