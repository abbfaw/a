...
class SaleOrderInherit(models.Model):
    ...
    def recup_all(self):
        ...
        retour = self.env['purchase.order'].sudo().search([('state', '=', 'purchase'), ('is_prive', '=', True), ('retour', '=', True)])
        for rt in retour:
            if rt:
                tracking = self.env['tracking.partner'].sudo().search([('purchase_ref', '=', str(rt.id) + 'ACH')])
                if tracking:
                    action = {
                        'partner': rt.partner_id.id,
                        'reference': rt.name,
                        'designation': 'Retour Achat',
                        'date': rt.create_date,
                        'purchase_ref': str(rt.id) + 'ACH',
                        'partner_type': 'vendor',
                        'purchase_id': rt.id,
                        'amount_currency': -rt.amount_total,
                    }
                    tracking.sudo().write(action)
                else:
                    action = {
