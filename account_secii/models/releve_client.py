...
class SaleOrderInherit(models.Model):
    ...
    def recup_all(self):
        ...
        p_comptable = self.env['account.move.line'].sudo().search(
            [('move_id', '=', 3), ('parent_state', '=', 'posted'), ('account_id.code', '=', 411100)])
        for pc in p_comptable:
            tracking = self.env['tracking.partner'].sudo().search([('purchase_ref', '=', str(pc.id) + 'PC')])
            print('TRK', tracking)
            action = {
            ...
            if tracking:
                print('Pièces comptables dupliquées')
                # tracking.sudo().write(action)
            else:
                print('Nouvelles Pièces comptables')
                # tracking.sudo().create(action)
        ...
        for vt in vente:
            if vt:
                ...
                action = {
                    ...
                    'reference': vt.name,
                    'designation': 'Vente',
                    'libele_op': vt.numero_bon,
                    'date': vt.date_effective,
...
class PurchaseOrderInherit(models.Model):
    ...
    def recup_purchase(self):
        res = self.env['purchase.order'].sudo().search(
            [('state', '=', 'purchase'), ('retour', '=', True), ('is_prive', '=', True)])
        print('test')
        for po in res:
