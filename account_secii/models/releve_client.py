        ...
        for dt in self:
            ...
            if dt.client:
                montant_cumule = self.env['sale.order'].sudo().search(
                    [('partner_id', '=', dt.client.id), ('state', 'in', ('sale', 'done'))]).mapped('amount_total')
            ...
            if val.client:
                cumul = self.env['account.secii'].sudo().search(
                    [('beneficiaire_is', '=', 'client'), ('beneficiaire_is_fournisseur', '=', val.client.id),
                     ('status', '=', 'valide'),
                     ('check_in_out', '=', 'entrer')]).mapped('somme')
                print('cumul', cumul)
                y = len(cumul)
                print('y', y)
                for csp in cumul:
...
class SaleOrderInherit(models.Model):
    ...
    # hérité de l'action dans un autre domaine et ajouter d'autres instructions
...
class AccountPaymentRegister(models.TransientModel):
    ...
    def action_create_payments(self):
        ...
        search = self.env['account.secii'].sudo().search([('beneficiaire_is_fournisseur', '=', pay.partner_id.id), ('date_prevue', '=', pay.invoice_date)])
