...
class SaleOrderInherit(models.Model):
    ...
    def recup_all(self):
        ...
        print('Achats')
        achat = self.env['purchase.order'].sudo().search([('state', '=', 'purchase'), ('is_prive', '=', True)])
        for ach in achat:
            if ach:
...
class PurchaseOrderInherit(models.Model):
    ...
    #     print('PO', purchase_order.search([]))
    #     for po in purchase_order.search([]):
    #         rt = self.env['secii.vente'].sudo().search(
    #             [('partner_id', '=', po.partner_id.id), ('name', '=', po.name)])
    #         for elt in rt:
    #             """Suppresion des données"""
    #             # elt.unlink()
    #         res = purchase_order.search([('partner_id', '=', po.partner_id.id), ('state', '=', 'purchase'), ('retour', '=', True), ('is_prive', '=', True)])
    #         for x in res:
    #             checking = self.env['secii.synthese'].sudo().search([('client', '=', x.partner_id.id), ('syn_id', '=', ('SALE' + str(x.id)))])
    #             print('CHK', checking)
                # if checking:
                #     action = {
                #         'syn_id': 'PURCHASE' + str(x.id),
                #         'sale_ids': [(0, 0, {
                #             'name': x.name,
                #             'date_order': x.date_order,
                #             'partner_id': x.partner_id.id,
                #             'create_uid': x.create_uid.id,
                #             'user_id': x.user_id.id,
                #             'amount_total': -x.amount_total,
                #             'state': 'purchase'
                #         })]
                #     }
                #     checking.sudo().update(action)
