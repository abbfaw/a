...
class AgentisComptable(models.Model):
    ...
    check_in_out = fields.Selection(
        [('entrer', 'Entrée'), ('sortie', 'Sortie'), ('yop', 'Transfert vers Caisse YOP'),
         ('transfer', 'Transfert vers Banque')],
        'Entrée/Sortie', default='sortie')
    somme = fields.Float(string='Somme:', track_visibility='always')
    ...
    status = fields.Selection(
        [('brouillon', 'BROUILLON'), ('annule', 'ANNULE'), ('attente', 'EN ATTENTE'), ('valide', 'VALIDE'),
    ...
    somme_lettre = fields.Char(string='Somme lettre', track_visibility='always')
    ...
    def action_open_transfer_popup(self):
        if len(self) > 1:
            action = {'type': 'ir.actions.act_window', 'name': 'Transfer de fond', 'view_mode': 'form', 'target': 'new',
                      'res_model': 'transfer.fond', 'view_id': self.env.ref('account_secii.view_transfer_fond_form').id,
                      }
        else:
            context = {'default_amount': abs(self.somme), 'default_libele': self.libele,
                       'default_partner_id': self.beneficiaire_is_fournisseur.id,
                       'default_type_operation': self.type_operation}
            action = {'type': 'ir.actions.act_window', 'name': 'Transfer de fond', 'view_mode': 'form', 'target': 'new',
                      'res_model': 'transfer.fond', 'view_id': self.env.ref('account_secii.view_transfer_fond_form').id,
                      'context': context}
        return action
    ...
    def get_product_id(self):
        sec = self.env['product.product'].sudo().search([('barcode', '=', 123456789)]).id
        print('sec', sec)
    ...
    def create(self, vals):
        ...
        if (in_out == 'sortie' or in_out == 'transfer' or in_out == 'yop') and not vals['etat']:
            if vals['somme'] != 0:
    ...
    def write(self, vals):
        ...
        if (self.check_in_out == 'sortie' or self.check_in_out == 'yop') and 'somme' in vals:
            if vals['somme'] > 0:
                vals['somme'] = - vals['somme']
        vals['update_by'] = self.env.uid
        vals['update'] = self.generate_update()
    ...
    def unlink(self):
        for val in self:
            if val.status not in ('brouillon','annule'):
                raise ValidationError(
                    _("Vous ne pouvez pas supprimer une opération comptabilisée ou payée, veuillez mettre en brouillon d'abord !"))
            if val.check_in_out == 'yop':
                yop = self.env['account.yop'].sudo().search([('caisse_id', '=', 'TRAN' + str(val.id))])
                if yop:
                    yop.status = 'brouillon'
                    yop.unlink()
    ...
    def put_suspense(self):
        ...
        self.status = 'attente'
        for val in self:
            val.message_post(body="statut ==> En attente")
        return True
    ...
    def put_cancel(self):
        ...
        self.status = 'annule'
        for val in self:
            val.message_post(body="statut ==> Annulé")
        return True

    def reput_validate(self):
        print('self ------', self)
        self.status = 'valide'
        for val in self:
    ...
    # recuperation de l'employée présent dans res.partner
    ...
    # @api.depends('beneficiaire_employee')
    ...
    # fonction pour passé le statut en brouillon et
    # empecher la mise en brouillon d'un transaction comptabilisé
    def put_draft(self):
    ...
    # fonction pour generer un numero de bon
    def generate_num_bon(self):
        init = 'COM'

    # fonction pour ouvrir les factures
    ...
    # fonction pour ouvrir les paiements
    ...
    def open_encaissement(self):
        ...
        domain = ([('date', '=', self.create_date), ('partenaire', '=', self.beneficiaire_is_fournisseur.id),
                   ('somme_paye', '=', self.somme)])
        action['domain'] = domain
        return action
    ...
    def open_commission(self):
        ...
        domain = ([('caisse_id', '=', 'COMMI' + str(self.id))])
        action['domain'] = domain
        return action
    ...
    # fonction pour mettre le statut en validé
    # et écrire dans la caisse de Yopougon
    def put_validate(self):
        ...
        for val in self:
            ...
            if yop:
                ...
                for va in vals:
                    va.update({'caisse_id': 'TRAN' + str(val.id), 'from_first': 'coc',
                               'somme': abs(va.get('somme')), 'check_in_out': 'entrer'})
                    yop.create(va)
            val.message_post(body="statut ==> Validé")
        return True
    ...
    def retrieve_dashboard(self):
        ...
        office_manager = self.env['account.secii'].search([])
            ...
            if res_user.has_group('account_secii.secii_box_see_all'):
                if office.status == 'brouillon':
                    ...
                    if office.check_in_out == 'entrer':
                        all_in += office.somme
                    if office.check_in_out == 'sortie' or office.check_in_out == 'transfer' or office.check_in_out == 'yop':
