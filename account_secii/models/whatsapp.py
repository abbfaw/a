...
class WhatsappMessage(models.TransientModel):
    ...
    client = fields.Many2one('res.partner', string="Destinataire")
    mobile = fields.Char(related='client.mobile', string="N° de Téléphone")
    ...
    def remplir_msg(self):
        ...
        rec = self.env['res.partner'].sudo().search([('id', '=', self.client.id)])
    ...
    def send_message(self):
        return {
            ...
            'url': "https://api.whatsapp.com/send?phone=+225" + self.client.mobile + "&text=" + self.message,
            'target': 'new',
            'res_id': self.id,
        }
