...
class SeciiEncaissement(models.Model):
    ...
    def _default_datetime_utc(self):
        locale_time = datetime.now()
        dt_utc = locale_time
        print('DT UTC', dt_utc)
        # return dt_utc
    ...
    # date_effective = fields.Datetime(string='Date Effective', compute='recup_date', tracking=True, store=True)
    date_effective = fields.Datetime(string='Date Effective', tracking=True, store=True)
    ...
    credit_client = fields.Float(string="Credit client")
    ...
    # def recup_date(self):
    #     enc = self.env['secii.encaissement'].sudo().search([])
    #     for elt in enc:
    #         elt.date_effective = elt.write_date
    ...
    def get_rest_topaye(self):
        ...
        for record in self:
            total_cumul = 0
            if record.instance:
                print('Reste à payer avec instance')
                cumul = self.env['secii.encaissement'].sudo().search(
                    [('partenaire', '=', record.partenaire.id), ('instance', '=', True)]).mapped('somme_paye')
                ...
                if record.montant_total == 0:
                    ...
                    if record.somme_paye < 0:
                        ...
                        print('Credit Client ')
                        record.credit_client = abs(record.montant_total - record.cumul)
                        record.partenaire.credit_client = record.credit_client
                        record.credit = 0
                    else:
                        print('==== reste à payer ====')
                        record.credit = abs(record.montant_total - record.cumul)
            else:
                print('Reste à payer sans instance')
                cumul = self.env['account.payment'].sudo().search(
                    [('partner_id', '=', record.partenaire.id), ('payment_type', '=', 'inbound')]).mapped('amount')
                print('CML', cumul)
                for x in cumul:
                    total_cumul += x
                    print('TCML', total_cumul)
                record.cumul = total_cumul + record.somme_paye
                if record.montant_total == 0:
                    record.credit = 0
                else:
    ...
    @api.onchange('partenaire', 'instance')
    def recherche_montant_facture(self):
        ...
        for dt in self:
            ...
            if dt.partenaire:
                if dt.instance:
                    print('Montant des devis avec instance')
                    montant_cumule = self.env['sale.order'].sudo().search(
                        [('partner_id', '=', dt.partenaire.id), ('state', 'in', ('sale', 'done')),
                    ...
                    for xz in montant_cumule:
                        dt.montant_total += xz
                # else:
                #     print('Montant des paiements')
                #     montant_cumule = self.env['account.payment'].sudo().search(
                #         [('partner_id', '=', dt.partenaire.id), ('caisse_id', '=', 'PAI' + str(self.id)),
                #          ('payment_type', '=', 'inbound')]).mapped('amount')
                #     print('montant cumule des paiements :', montant_cumule)
                #     x = len(montant_cumule)
                #     print('x', x)
                #     for xz in montant_cumule:
                #         dt.montant_total += xz
    ...
    def comptabilise_operation(self):
        ...
        for payment in self:
            ...
            if duplicate:
                ...
                self.env['account.payment'].maj_tracking_partner_payment()  # créer le relévé dans account.bank.statement
    ...
    def creation_rapport(self):
        ...
        for val in self:
            ...
            """vérification si le vue consolidée a déjà les encaissements"""
            tracking = self.env['tracking.partner'].sudo().search([('purchase_ref', '=', str(val.id) + 'ENC')])
            ...
            if val.instance:
                if tracking:
                    ...
                    action = {
                        'partner': val.partenaire.id,
                        'reference': val.num_seq,
                        'designation': 'Encaissement',
                        'libele_op': val.libele_op,
                        'date': val.date,
                        'purchase_ref': str(val.id) + 'ENC',
                        'partner_type': 'customer',
                        'amount_currency': - val.somme_paye,
