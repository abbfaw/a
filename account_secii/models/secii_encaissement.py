...
class SeciiEncaissement(models.Model):
    ...
    def onchange_partenaire_id(self):
        print('rec.partenaire.id =', self.partenaire.id)
        print('rec.partenaire =', self.partenaire)
        self.commercial = self.env['hr.employee'].sudo().search([('user_id', '=', self.partenaire.user_id.id)])
        self.old_com = self.env['hr.employee'].sudo().search([('user_id', '=', self.partenaire.user_id_old)])
        print(self.old_com.id)
        for rec in self:
            return {'domain': {'amount_total': [('partner_id', '=', rec.partenaire.id)],
                               'commercial': [('user_id', '=', rec.partenaire.user_id.id)]}}
    ...
    # @api.onchange('partenaire')
    # def commercial_client(self):
    #     for ret in self:
    #         print('partnaireeeeeeeeeeeeeee')
    #         return {'domain': {'commercial': [('user_id', '=', ret.partenaire.user_id.id)]}}
    ...
    # empecher la suppression des données qui ont les statuts comptabilisé, payé
    # et le statut != annulé dans mouvement de caisse
    def unlink(self):
        for val in self:
        ...
        for dt in self:
            ...
            if dt.partenaire:
                montant_cumule = self.env['sale.order'].sudo().search(
                    [('partner_id', '=', dt.partenaire.id), ('state', 'in', ('sale', 'done'))]).mapped(
                    'amount_total')
    ...
    # def write(self, vals):
    #     print('Message doit être posté')
    #     for val in self:
    #         self.message_post(body="Nouvel encaissement crée")
    #     result = super(SeciiEncaissement, self).write(vals)
    #     return result
    ...
    # def _encaissement_filter(self):
    #     print('_get_actual_user =', self._get_actual_user())
    #     return {
    #         'type': 'ir.actions.act_window',
    #         'res_model': 'secii.encaissement',
    #         'views': [[self.env.ref('account_secii.encaissement_commerciaux_list').id, 'tree']],
    #         'view_id': self.env.ref('account_secii.encaissement_commerciaux_list').id,
    #         'view_type': 'tree',
    #         'view_mode': 'tree, form',
    #         'domain': [('status', '=', 'brouillon'), ('commercial', '=', self._get_actual_user())],
    #         'name': 'Encaissement',
    #     }
