...
class ProjectReportParser(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        all_orders = purchase.search([('partner_id', '=', partner_id), ('state', 'in', ('sale', 'done')),
                                      ...
                                      ('date_effective', '<=', end_date)]) #mettre create_date à la place de date_effective
        print('AO ->', all_orders)
        ...
        past_purchase = sum(purchase.search([('partner_id', '=', partner_id),
                                             ('state', 'in', ('sale', 'done')),
                                             ('date_effective', '<', start_date)]).mapped('amount_total'))
        ...
        for x in a:
            all_vals = {}
            var = purchase.sudo().search([("date_effective", '=', x), ('partner_id', '=', partner_id)]) #erreur si remplacer par date_effective
