...
class TrackingInstancePartner(models.Model):
    ...
    # amount_eur = fields.Monetary(string='Montant en €')
    # amount_usd = fields.Monetary(string='Montant en $')
    # amount_cf = fields.Monetary(string='Montant en CFA')
    currency_id = fields.Many2one('res.currency', string='Devise', default=lambda self: self.env.company.currency_id.id)
    amount_currency = fields.Char(string='Montant')
