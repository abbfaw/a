...
class PurchaseEncaissement(models.Model):
    ...
    def _compute_amount_total_signed(self):
        for val in self:
            ...
            val.amount_devise = str('{:,}'.format(float_round(- val.somme_paye, 2)).replace(',', ' ')) + ' ' + str(
        ...
        for val in self:
            ...
            if exist:
                if val.currency_id.name == 'XOF':
                    action.update({
                        # 'debit': float_round(
                        #     val.somme_paye * get_price_in_xof(val.currency_id, val.company_id, val.date), 2),
                        'amount_currency': str('{:,}'.format(float_round(val.somme_paye, 0)).replace(',',
                                                                                                     ' ')) + ' ' + val.currency_id.symbol,
                        # 'amount_cf': - val.somme_paye,
