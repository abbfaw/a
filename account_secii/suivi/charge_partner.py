...
def get_price_in_xof(currency):
    currency_name = currency.name
    if currency_name != 'XOF':
        cof = currency.rate_ids.inverse_company_rate
        return cof
    else:
        return 1
    ...
    def add_to_tracking(self):
        ...
        for val in self:
            ...
            if val.edit_rate > val.amount_taux:
                ...
                action.update({
                    # 'debit': val.change_line.amount_gain,
                    'amount_currency': str('{:,}'.format(
                        float_round(val.change_line.amount_gain, 0)).replace(',',
                                                                             ' ')) + ' CFA',
                    # 'amount_cf': - val.change_line.amount_gain,
                ...
                action.update({
                    # 'credit': val.change_line.amount_perte,
                    'amount_currency': str(
                        '{:,}'.format(float_round(val.change_line.amount_perte, 0)).replace(',',
                                                                                            ' ')) + ' CFA',
                    # 'amount_cf': val.change_line.amount_perte,
                    ...
                    line = (0, 0,
                            {
                                ...
                                'name': val.libele,
                                # 'debit': 0,
                                # 'credit': val.change_line.amount_gain
                            }
                            )
                    ...
                    line = (0, 0,
                            {
                                ...
                                'partner_id': val.partner.id,
                                'name': val.libele,
                                # 'debit': val.change_line.amount_perte,
                                # 'credit': 0
                            }
                            )
                    all_lines.append(line)
