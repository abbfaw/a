...
class PurchaseOrder(models.Model):
    ...
    def _compute_releve_compte_client(self):
        ...
        for val in self:
            action = {
                ...
                'reference': val.name,
                'designation': 'Achat',
                # 'ref_import': val.import_ref,
                # 'nature_operation': val.nature_produit,
            ...
            if val.is_prive:
                if val.currency_id.name == 'XOF':
                    action.update({
                        # 'credit': float_round(
                        #     val.amount_total * get_price_in_xof(val.currency_id, val.company_id, val.date_order.date()), 2),
                        'amount_currency': '{:,}'.format(
                            float_round(val.amount_total, 2)).replace(',',
                                                                      ' ') + ' ' + val.currency_id.symbol,
                        # 'amount_cf': float_round(val.amount_total, 2),
    ...
    def button_confirm(self):
        for line in self:
            # if line.add_price:
            #     raise ValidationError(
            #         _("Un achat ayant un prix officiel ne peut être ne peut pas être confirmé.Seul l'achat \"généré\" "
            #           "peut-être confirmé (générer achat additionnel) !"))
            # else:
            if line.is_prive:
        ...
        for va in val:
            if va.get('is_prive'):
                ...
                quotation = {
                    ...
                    'notes': va.get('notes'),
                    # 'import_ref': va.get('import_ref'),
                    # 'nature_produit': va.get('nature_produit'),
